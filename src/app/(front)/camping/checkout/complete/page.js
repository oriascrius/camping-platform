...
import { FaCheckCircle, FaCreditCard, FaMoneyBill, FaLine, FaFileAlt, FaUser, 
  FaCampground, FaCalendar, FaUsers, FaClock, FaHome, FaHistory, FaChevronDown, 
  FaChevronUp, FaShoppingCart, FaCheck } from 'react-icons/fa';
      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="text-center mb-10">
          <div className="relative inline-block mb-6">
            <div className="w-20 h-20 rounded-full bg-[var(--primary-brown)] 
              flex items-center justify-center mx-auto">
              <FaCheckCircle className="text-white text-4xl" />
            </div>
          </div>
          <h1 className="text-2xl font-bold text-[var(--primary-brown)] mb-2 m-0">訂單完成</h1>
          <p className="text-[var(--gray-3)] m-0">
            訂單編號：
            <span className="text-[var(--primary-brown)]">{orderData?.order_id || '0'}</span>
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden
              ${expandedSections.orderInfo ? 'max-h-[500px] opacity-100 py-6' : 'max-h-0 opacity-0 py-0'}`}>
              <div className="px-6">
                <div className="grid grid-cols-2 gap-4">
                  <InfoItem 
                    label="訂單日期" 
                    value={format(new Date(orderData.created_at), 'yyyy/MM/dd HH:mm')} 
                  />
                  <InfoItem 
                    label="付款方式" 
          ...
          <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden
              ${expandedSections.contactInfo ? 'max-h-[500px] opacity-100 py-6' : 'max-h-0 opacity-0 py-0'}`}>
              <div className="px-6">
                <div className="grid grid-cols-1 gap-6">
                  <div className="p-4 rounded-lg bg-[var(--lightest-brown)]">
                    <p className="text-[var(--gray-3)] text-sm mb-2 m-0">姓名</p>
                    <p className="text-[#AC7654] font-medium m-0">{orderData.contact_name}</p>
                  </div>

                  <div className="p-4 rounded-lg bg-[var(--lightest-brown)]">
                    <p className="text-[var(--gray-3)] text-sm mb-2 m-0">電話</p>
                    <p className="text-[#AC7654] font-medium m-0">{orderData.contact_phone}</p>
                  </div>

                  <div className="p-4 rounded-lg bg-[var(--lightest-brown)]">
                    <p className="text-[var(--gray-3)] text-sm mb-2 m-0">電子信箱</p>
                    <p className="text-[#AC7654] font-medium m-0">{orderData.contact_email}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden
              ${expandedSections.bookingItems ? 'max-h-[1000px] opacity-100 py-6' : 'max-h-0 opacity-0 py-0'}`}>
              <div className="px-6">
                <div className="space-y-4">
                  {orderData.items.map((item, index) => (
                    <div key={index} className="p-4 rounded-lg bg-[var(--lightest-brown)]">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-sm text-[var(--gray-3)] m-0">/ 每晚</p>
                        </div>
                      </div>
                ...
                <div className="mt-6 pt-4 border-t border-[var(--tertiary-brown)]">
                  <div className="flex justify-between items-center">
                    <span className="text-xl font-bold text-[var(--primary-brown)] m-0">總金額</span>
...
const InfoItem = ({ label, value, icon: Icon, color }) => (
  <div className="p-3 rounded-lg bg-[var(--lightest-brown)]">
    ...
    <p className={`font-medium flex items-center gap-2 ${color || 'text-[var(--primary-brown)]'} m-0`}>
      {Icon && <Icon />}
      {value}
    </p>
  </div>
...
const BookingDetail = ({ icon: Icon, label, value }) => (
  <div className="flex items-center gap-2 text-[var(--gray-3)]">
    ...
    <span className="m-0">{label}：{value}</span>
  </div>
);

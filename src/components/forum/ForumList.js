...
const ForumList = ({ setCategory }) => { // 接收 setCategory
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const validCategories = data.filter(category => category.id !== undefined);
    ...
    <div className="forumList">
      <div className="forumMenu d-flex justify-content-center">
        {categories.map((category) => (
          <div
            key={category.id}
            className={`forumMenuBtn ${activeMenu === category.id ? 'active' : ''}`}
